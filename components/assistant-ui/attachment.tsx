...
const AttachmentThumb: FC = () => {
  ...
  return (
    <Avatar className="aui-attachment-tile-avatar h-full w-full rounded-none">
      ...
      />
      <AvatarFallback>
        <FileTextIcon className="aui-attachment-tile-fallback-icon text-muted-foreground size-8" />
      </AvatarFallback>
    </Avatar>
  );
};
...
const AttachmentUI: FC = () => {
  ...
  const fileInfo = useAuiState((s) => ({
    name: s.attachment.file?.name ?? s.attachment.name,
    size: s.attachment.file?.size,
    mime: s.attachment.file?.type,
  }));
      ...
      >
        <AttachmentPreviewDialog>
          <TooltipTrigger render={<div className={cn(
                                  ...
                                  !isImage && "h-14 w-56 rounded-xl",
                                )} role="button" tabIndex={0} aria-label={`${typeLabel} attachment${
                                  isError ? ", upload failed" : isUploading ? ", uploading" : ""
                                }`} />}>{isImage ? <AttachmentThumb /> : <div className="flex h-full items-center gap-3 px-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-background"><FileTextIcon className="size-5 text-muted-foreground" /></span><span className="min-w-0 text-left"><span className="block truncate text-xs font-medium text-foreground">{fileInfo.name || "未命名文件"}</span><span className="block truncate text-[10px] text-muted-foreground">{fileInfo.mime || typeLabel}{fileInfo.size ? ` · ${(fileInfo.size / 1024).toFixed(fileInfo.size > 1024 * 1024 ? 0 : 1)} KB` : ""}</span></span></div>}{isUploading && (
                                  <div
                                    aria-hidden="true"
                                  ...
                                  >
                                    <CircleNotchIcon className="text-muted-foreground size-5 animate-spin" />
                                  </div>
                                )}{isError && (
                                  <div
                                    aria-hidden="true"
